...
__all__ = ['modify_axes', 'init_figure', 'plot_bands', 'add_text', 'quick_bplot', 'sbands', 'add_legend',
           'add_colorbar', 'color_wheel', 'get_pros_data', 'make_line_collection', 'plot_collection', 'quick_rgb_lines',
           'srgb', 'quick_color_lines', 'scolor', 'select_pdos', 'collect_dos', 'quick_dos_lines', 'sdos', 'plt2html',
...
# Cell
def plot_bands(ax=None,kpath=None,bands=None,showlegend=False,E_Fermi=None,interp_nk = {},**kwargs):
    """
...
# Cell
def quick_bplot(path_evr=None,ax=None,skipk=None,kseg_inds=[],elim=[],ktick_inds=[],ktick_vals=[],E_Fermi=None,txt=None,xytxt=[0.2,0.9],ctxt='black',interp_nk = {}, **kwargs):
    """
    - Returns axes object and plot on which all matplotlib allowed actions could be performed.
    - **Parameters**
    ...
    plot_bands(ax=ax,kpath=K,bands=vr.bands,showlegend=True,interp_nk = interp_nk, E_Fermi=E_Fermi,**kwargs)
    text = txt if txt != None else vr.sys_info.SYSTEM
    add_text(ax,*xytxt,text,colors=ctxt)
    return ax
sbands = quick_bplot #Alias
...
def quick_color_lines(path_evr      = None,
    ...
    plt.subplots_adjust(**subplots_adjust_kwargs)
    return axes
scolor = quick_color_lines # Alias
