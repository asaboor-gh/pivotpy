...
__all__ = ['plot_bands', 'modify_axes', 'quick_bplot', 'add_text', 'add_legend', 'add_colorbar', 'color_wheel',
           'get_pros_data', 'make_line_collection', 'plot_collection', 'create_rgb_lines', 'quick_rgb_lines',
           'quick_color_lines', 'init_figure', 'select_pdos', 'collect_dos', 'quick_dos_lines', 'plt_to_html',
           'plot_potential']
...
def get_pros_data(kpath      = None,
    ...
    - **Returns**
        - A dictionary with keys 'kpath', 'evals', 'colors' that can be unpacked in next function's arguemnet.
            ...
            - 'colors': An array of shape (NKPTS,NBANDS,n) where n is length of input elements list.
    """
    import numpy as np
    ...
    for elem, orb in zip(elements,orbs):
        if not elem or not orb:
            raise ValueError("elements and orbs both should not be empty.")
    ...
    if(interpolate==True):
        ...
        return {'kpath':knew,'evals': evals,'colors': _colors}
    # Return if no interpolation
    return {'kpath':kpath,'evals': evals_set,'colors': _colors}
...
def make_line_collection(max_width   = 2.5,
                         ...
                         **pros_data):
    """
    - Returns a tuple of line collections. If rgb = True (at len(orbs) = 3 in `get_pros_data`), returns multicolored line collection, otherwise return single colored multiple lines.
    - **Parametrs**
    ...
    if not pros_data:
        ...
        colors = pros_data.get('colors')
    req_args = [kpath, evals, colors]
    for a,t in zip([kpath, evals, colors],['kpath', 'evals', 'colors']):
        if not np.any(a):
            return print("Missing {!r} from output of `get_pros_data()`".format(t))

    # Average color on two consecutive KPOINTS
    colors = colors[1:,:,:]/2 + colors[:-1,:,:]/2 # Near kpoints avearge
    colors = colors.transpose((1,0,2)).reshape((-1,np.shape(colors)[-1])) # Must before lws

    # Only scale data when sacle_color is False, no need of double operation
    if scale_data and not scale_color: # Normalize overall data
        c_max = np.max(colors)
...
def quick_color_lines(path_evr      = None,
    ...
    - **Parameters**
        ...
        - scale_data : If True, normalize projection data to 1.
        - max_width  : Width to scale whole projections. if `uni_width=True, width=max_width/2`.
        - figsize    : Tuple (width,height) in inches. Default (3.4.2.6) is article column's width.
        - txt        : Text on figure, if None, SYSTEM's name is printed.
        - xytxt      : [x_coord,y_coord] of text relative to axes.
    ...
    for i,elem in enumerate(elements.copy()):
        if type(elem) == int:
            try:
                elements[i] = range(elem_inds[elem],elem_inds[elem+1])
                info = """
                elements[{}] = {} is converted to {} which picks all ions of {!r}.
                """.format(i,elem,elements[i],vr.sys_info.ElemName[elem])
    ...
    gpd_args = dict(elements=elements,orbs=orbs,interpolate=interpolate,n=n,k=k)
    mlc_args = dict(rgb=False,uni_width=False,scale_color = False,colors_list= colors, max_width=max_width,scale_data=scale_data)
    ...
    if ISPIN==2:
        ...
        if spin=='both':
            labels = [l+'$^{↑↓}$' for l in labels if l]
    if showlegend:
        pp.add_legend(ax=axes[0],colors=colors,labels=labels,widths=max_width/2,**legend_kwargs)
    else:
        x,y=[*xytxt]
        _ = [pp.add_text(ax=ax,xs=x,ys=y,txts=label,colors=ctxt) for ax,label in zip(axes,labels)]
