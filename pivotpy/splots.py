...
__all__ = ['modify_axes', 'get_axes', 'append_axes', 'join_axes', 'break_spines', 'add_text', 'splot_bands',
           'add_legend', 'add_colorbar', 'color_wheel', 'color_hex', 'splot_rgb_lines', 'splot_color_lines',
...
def get_axes(figsize  = (3.4,2.6),
    ...
    try:
        for ax in np.array([axs]).flatten():
            for f in [add_text,add_legend,add_colorbar,color_wheel,color_hex, break_spines,modify_axes,append_axes]:
                if ax.name != '3d':
...
def color_hex(ax, colormap = 'brg', loc = (0.67,0.67), size=0.3, N = 7, labels=['s','p','d']):
    ...
    colors = [C[t].mean(axis=0) for t in tr1.triangles]
...
def splot_rgb_lines(
    ...
    def _add_collection(gpd_args,mlc_args,ax):
        ...
        if len(non_zero_inds) == 1:
            ...
            # Normalize after picking colors from colormap as well to match the color_hex.
    ...
    ax.pro_labels = _tls_  # Save labels for later use in color_hex
    if colorbar:
        if len(non_zero_inds) ==  1:
