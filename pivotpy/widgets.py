...
__all__ = ['css_style', 'dark_colors', 'light_colors', 'simple_colors', 'get_files_gui', 'InputGui', 'generate_summary',
           'VasprunApp', 'KPathApp', 'intslider_html', 'LiveSlides', 'init_slides', 'collect_slides',
           'new_slides_notebook']
...
# Cell
def intslider_html(slider, fill_color = '#61afef', empty_color = 'whitesmoke'):
...
# Cell
def init_slides(number_of_slides,new_notebook=False):
    """Helper function to create placehoder code cells for slides in Notebook. You can manually create a cell with %%capture __slide_[number] as slide."""
    if number_of_slides < 1:
        return print('Provide Number of Slides >= 1')
    ...
    # Cells are written in reverse order.
    cells_reversed = []
    cells_reversed.append('This is empty cell to replace input cell. Very important')
    end_cell = """
                        ...
                        slides_iterable = pp.collect_slides() #Get all slides content in order

                        def display_item(item):
                            if isinstance(item,(str,dict,list,tuple,int)):
                                display(Markdown(f'### Given {{type(item)}}: {{item}}'))
                            else:
                                item.show() # displays output of %%capture

                        ls = pp.LiveSlides(func=display_item,iterable=slides_iterable, title_page_md=title_page_md,accent_color='red')
                        ls.show()"""
    cells_reversed.append(textwrap.dedent(end_cell))
    cells_reversed.append(textwrap.dedent("""
                                # There will be 4 slides after slide 1 dynamically generated. Modify display_item to handle each type you give.
                                __next_to_1 = ['This is String', 2, {'A':'Dictionary'},(1,2,3)]"""))

    for i in range(1,number_of_slides):
        sn = number_of_slides - i + 1
        cells_reversed.append(f'%%capture __slide_{sn}\n# ---- Slide {sn} ----\ndisplay(Markdown("# Slide {sn} Title"))')
    ...
    guid_lines += f'\n\n ## Code that Generates Slides\n\n```python\n{textwrap.dedent(end_cell)}\n```\n'

    cells_reversed.append(f'%%capture __slide_1\n# ---- Slide 1 ----\ndisplay(Markdown("""# Slide 1 Title\n### There is a plot"""))'+some_code)
    cells_reversed.append('# ------ Slides Start Here --------\n# Each slide should have %%capute __slide_[number]__ in top of cell')
    cells_reversed.append(f'from IPython.display import display, Markdown\n\ntitle_page_md=""" # Interactive Slides  \n<em style="color:red"> Author: Abdul Saboor</em>{guid_lines}"""' )
    cells_reversed.append('''%%html
    \n<style>\n:root {\n\t--heading-fg: red;\n\t--text-fg: blue;\n\t--text-bg: inherit;\n\t --quote-bg: inherit;\n\t--quote-fg: green;\n}
    ...
    # If write in current Notebook
    if not new_notebook:
        for cell in cells_reversed:
            create_new_cell(cell)

        current_cell_code = shell.get_parent()['content']['code'].splitlines()
        code_after = '\n'.join([line for line in current_cell_code if 'init_slides' not in line])
        shell.set_next_input(code_after, replace= True)
    else:
        import nbformat
        from nbformat import v4 as nbf
        nb = nbf.new_notebook()
        cells = [nbf.new_code_cell(cell) for cell in cells_reversed[1:][::-1]] #Do not add placeholder cell
        nb.cells.extend(cells) #Make order down
        with open(gu.prevent_overwrite('live-presentation.ipynb'), 'w') as f:
            nbformat.write(nb, f)

def collect_slides():
    """Collect cells with variables `__slide_[N]` and `__next_to_[N]` in user's namespace."""
    ns = get_ipython().user_ns
    fixed_slides = {k: v for k,v in ns.items() if '__slide_' in k}
    dynamic_slides = {k: v for k,v in ns.items() if '__next_to_' in k}
    # If slide number is mistaken, still include that.
    max_slides = max([int(k.split('_')[-1]) for k in [*fixed_slides.keys(), *dynamic_slides.keys()]])
    slides_iterable = []
    for i in range(1,max_slides + 1):
        if f'__slide_{i}' in fixed_slides:
            slides_iterable.append(fixed_slides[f'__slide_{i}'])
        if f'__next_to_{i}' in dynamic_slides:
            slides_iterable = [*slides_iterable,*dynamic_slides[f'__next_to_{i}']]
    return tuple(slides_iterable)

def new_slides_notebook(number_of_slides):
    "Creates new notebook for prsentation with pre-defined number of slides."
    init_slides(number_of_slides=number_of_slides, new_notebook=True)