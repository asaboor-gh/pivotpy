...
__all__ = ['css_style', 'dark_colors', 'light_colors', 'get_files_gui', 'get_input_gui', 'click_data', 'tabulate_data',
           'save_data', 'color_toggle', 'generate_summary', 'VasprunApp', 'theme_editor']
...
class VasprunApp:
    ...
    def __init__(self,height=600):
        ...
        self.theme_colors = light_colors
...
# Cell
def theme_editor():
    """Edit colors and use in `app_name.theme_colors` for custom theme."""
    colors_dict = light_colors.copy()
    _dd = Dropdown(options = [k for k,v in colors_dict.items()])
    _picker = ipw.ColorPicker()
    _html = ipw.HTML()
    out = ipw.Output().add_class('output')
    @out.capture(clear_output=True,wait=True)
    def update_color(change):
        colors_dict[_dd.value] = _picker.value
        print("theme_colors = ",json.dumps(colors_dict,indent=4))
        _html.value = css_style(colors_dict)
    _picker.observe(update_color,"value")
    return HBox([VBox([_html,_dd, _picker,Text(),Button(description='btn_fg')]),out])