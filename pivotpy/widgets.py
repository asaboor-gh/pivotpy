...
__all__ = ['light_style', 'dark_style', 'get_files_gui', 'get_input_gui', 'read_data', 'click_data', 'tabulate_data',
           'save_data', 'color_toggle', 'clrear_cache', 'matplotlib_code', 'show_app']
...
def show_app():
    ...
    theme_w = ipw.Dropdown(options=['Default','Light','Dark'],value='Light',layout=b_out)
    ...
    # Monochrome/ Colorize
    tog_w=ipw.Button(description='Monochrome',button_style='primary')
    def tog_b(tog_w):
        color_toggle(tog_w,fig)
    tog_w.on_click(tog_b)

    # Save Offline
    save_fig_w = ipw.Button(description='Save Fig')
    ...
    confirm_btn = ipw.Button(description='Confirm Delete',button_style='danger')
    def deleter(btn):
    ...
    mpl_btn = ipw.Button(description='Grenerate Matplotib Code',layout=Layout(width='max-content'))
    def mpl_code(btn):
        mpl_btn.description = 'Generating...'
        string = matplotlib_code(rd_btn,out_w1,dict_html)
        get_ipython().set_next_input(string)
        mpl_btn.description = 'Run next cell to get code'
        mpl_btn.style.button_color = 'cyan'
        from time import sleep
        sleep(2)
    ...
    left_box = VBox([upper_box,in_box,edit_box,fig_otions, cache_box,mpl_btn],layout=Layout(width='40%')).add_class('marginless').add_class('borderless')
