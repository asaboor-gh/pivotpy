...
__all__ = ['Arrow3D', 'fancy_quiver3d', 'write_poscar', 'export_poscar', 'InvokeMaterialsProject', 'get_kpath',
           'read_ticks', 'str2kpath', 'get_kmesh', 'cartesian_kmesh', 'order', 'rotation', 'get_bz', 'splot_bz',
           'iplot_bz', 'to_R3', 'kpoints2bz', 'fix_sites', 'get_pairs', 'iplot_lat', 'splot_lat', 'join_poscars',
...
def cartesian_kmesh(X,Y,Z,weight = None, ibzkpt= None, outfile=None):
    """X,Y,Z should be output of numpy.meshgrid in 3D. Just like X, Y, Z = np.mgrid[-1:1:10j, -1:1:10j, -1:1:5j] for example.
    KPOINTS is generated as cartesian so should span full cell size when multiplied by 2π/a where a is scaling factor in POSCAR."""
    _X = X.flatten()
    _Y = Y.flatten()
    _Z = Z.flatten()
    N = len(_X)
    ...
    out_str = '\n'.join(f"{x:>16.10f}{y:>16.10f}{z:>16.10f}{weight:>12.6f}" for x,y,z in zip(_X,_Y,_Z))
