...
def atoms_color(show_plot = True):
    "Display color of elements of periodic table on a grid."
    if show_plot == False:
        return _atom_colors
    array = np.ones((121,3))
    names = ['' for i in range(121)]
    for i,(k,v) in enumerate(_atom_colors.items()):
        array[i] = v
        names[i] = k

    array = np.reshape(array,(11,11,3))
    names = np.reshape(names,(11,11))
    ...
    ax.set_yticklabels([str(i) for i in range(0,121,11)])
    ax.xaxis.tick_top()
    _ = ax.set_title('Atom Number = Row + Column')
    plt.show()
