...
__all__ = ['atoms_color', 'Arrow3D', 'fancy_quiver3d', 'write_poscar', 'export_poscar', 'InvokeMaterialsProject',
           'get_kpath', 'read_ticks', 'str2kpath', 'get_kmesh', 'order', 'rotation', 'get_bz', 'splot_bz', 'iplot_bz',
           'to_R3', 'to_basis', 'kpoints2bz', 'fix_sites', 'get_pairs', 'iplot_lat', 'splot_lat', 'join_poscars',
           'scale_poscar', 'rotate_poscar']
...
_atom_colors = {'H': (0.7, 0.8, 0.7), 'He': (0.8367, 1.0, 1.0),
    'Li': (0.7994, 0.9976, 0.5436), 'Be': (0.7706, 0.0442, 0.9643), 'B': (1.0, 0.5, 0), 'C': (0.4, 0.4, 0.4), 'N': (0.292, 0.8886, 0.438), 'O': (0.8005, 0.1921, 0.2015), 'F': (0.5785, 0.4089, 0.8554), 'Ne': (0.6773, 0.9553, 0.9284),
    'Na': (0.6587, 0.8428, 0.4922),'Mg': (0.6283, 0.0783, 0.8506),'Al': (175/255, 175/255, 175/255),'Si': (248/255, 209/255, 152/255),'P': (1.0, 0.0, 0.502),'S': (0.9044, 0.135, 0.9702),'Cl': (0.4127, 0.1664, 0.9327),'Ar': (0.5461, 0.8921, 0.8442),
    'K':  (0.534, 0.7056, 0.4207), 'Ca': (0.4801, 0.0955, 0.7446), 'Sc': (0.902, 0.902, 0.902), 'Ti': (0.749, 0.7804, 0.7608), 'V': (0.651, 0.6706, 0.651), 'Cr': (0.5412, 0.7804, 0.6), 'Mn': (0.6118, 0.7804, 0.4784), 'Fe': (0.8784, 0.2, 0.4),
    'Co': (0.9412, 0.6275, 0.5647), 'Ni': (0.3137, 0.3137, 0.8157), 'Cu': (184/255, 115/255, 51/255), 'Zn': (186/255, 196/255, 200/255),'Ga': (149/255, 191/255, 214/255),'Ge': (0.6051, 0.5765, 0.6325),'As': (0.7412, 0.8902, 0.502),'Se': (0.9172, 0.0707, 0.6578),
    'Br': (0.5885, 0.1606, 0.2216),'Kr': (0.426, 0.8104, 0.7475),'Rb': (0.4254, 0.5859, 0.3292),'Sr': (0.326, 0.096, 0.6464),'Y': (0.531, 1.0, 1.0),'Zr': (0.4586, 0.9186, 0.9175),'Nb': (0.385, 0.8417, 0.8349),'Mo': (0.3103, 0.7693, 0.7522),
    'Tc': (0.2345, 0.7015, 0.6694), 'Ru': (0.1575, 0.6382, 0.5865), 'Rh': (0.0793, 0.5795, 0.5036), 'Pd': (0.0, 0.5252, 0.4206), 'Ag': (0.7529, 0.7529, 0.7529), 'Cd': (0.8,0.67,0.73), 'In': (0.7284, 0.4222, 0.4406), 'Sn': (0.398, 0.4956, 0.4915),
    'Sb': (0.6196, 0.7098, 0.3882), 'Te': (0.8167, 0.0101, 0.4513), 'I': (0.5804, 0.5804, 0.0), 'Xe': (0.3169, 0.7103, 0.6381), 'Cs': (0.3328, 0.4837, 0.2177), 'Ba': (0.1659, 0.0797, 0.556), 'La': (0.9281, 0.3294, 0.7161), 'Ce': (0.8948, 0.3251, 0.7314),
    ...
    'Bi': (0.6196, 0.7098, 0.3098), 'Po': (0.6706, 0.0, 0.3608), 'At': (0.4588, 0.2706, 0.3098), 'Rn': (0.2188, 0.5916, 0.5161), 'Fr': (0.2563, 0.3989, 0.0861), 'Ra': (0.0, 0.0465, 0.4735), 'Ac': (0.322, 0.9885, 0.7169), 'Th': (0.3608, 0.943, 0.6717),
...
def atoms_color(show_plot = True):
    "Display color of elements of periodic table."
    if show_plot == False:
        return _atom_colors
    array = np.ones((180,3))
    names = ['' for i in range(180)]
    shift = 0
    for i,(k,v) in enumerate(_atom_colors.items()):
        array[i+shift] = v
        names[i+shift] = f'$^{{{str(i+1)}}}${k}'
        if i == 0:
            shift += 16
        elif i in [3,11]:
            shift += 10
    ...
    _array = array[5:,:,:].reshape((-1,3))
    _names = names[5:,:].ravel()

    # lanthanides and actinides
    array[5,:3] = _array[:3]
    names[5,:3] = _names[:3]
    array[6,:3] = _array[32:35]
    names[6,:3] = _names[32:35]
    array[5:7,3:,:] = [1,1,1]

    array[7:9,:] = [1,1,1]
    names[7:9] = ''

    _copy_names = [f'$^{{{str(i+1)}}}${k}' for i,k in enumerate(_atom_colors.keys())]
    _copy_array = list(_atom_colors.values())

    array[5,3:] = _copy_array[71:86]
    names[5,3:] = _copy_names[71:86]

    array[6,3:] = _copy_array[103:]
    names[6,3:] = _copy_names[103:]

    array[8,3:17] = _copy_array[57:71]
    names[8,3:17] = _copy_names[57:71]
    array[9,3:17] = _copy_array[89:103]
    names[9,3:17] = _copy_names[89:103]

    ax = sp.get_axes((9,4.5))
    ax.imshow(array)
    for i in range(18):
        for j in range(10):
            plt.text(i,j,names[j,i],color = 1 - array[j,i],ha='center',va='center')
    ax.set_axis_off()
    plt.show()
